...
run_simulation <- function(data, num_simulations = 1000) {
  ...
  for (i in 1:num_simulations) {
    ...
    # Fit the ps model. Using "full" method as per your reasoning
    match_model <- matchit(formula, data = data, method = "full", distance = "logit", estimand = "ATT")
    
    # *** Debugging print -- check the summary of the match model
    #print(summary(match_model)) 
    
    # Calculate ATT
    att <- NA
    if (!is.null(summary(match_model)$sum.matched)) {
    ...
    # Debugging print *** -- cov balance
    print(str(summary(match_model)$covariate.balance)) 
    
    # Debugging print *** -- check the balance data
    # print(smd_data)  # Check if this is NULL or empty
    
    if (!is.null(smd_data) && nrow(smd_data) > 0) {
      # Mark covariates with SMD <= 0.1 as balanced
      smd <- smd_data %>% 
        mutate(balanced = smd <= 0.1)  # SMD <= 0.1 considered balanced
      ...
      balance_plot <- ggplot(smd, aes(x = covariate, y = smd)) +
        geom_bar(stat = "identity", fill = ifelse(smd$smd <= 0.1, "blue", "red")) +
        coord_flip() + 
        theme_minimal() + 
...
# troubleshooting function with small sample
test_data <- ypsps_clean %>% sample_n(100)  # Take a small random sample
simulation_test <- run_simulation(test_data, num_simulations = 5)

#glimpse(simulation_test)


# Extract results -- sample
simulation_results <- simulation_test$results
...
run_simulation <- function(data, num_simulations = 1000) {
  ...
  for (i in 1:num_simulations) {
    ...
    match_model <- matchit(formula, data = data, method = "full", distance = "logit", estimand = "ATT")
   # debugging print -- don't run with full sim
   # print(summary(match_model)) # low
    
    #debugging print -- don't run with full sim
    #print(summary(match_model)$sum.matched)

    # Calculate ATT
    att <- summary(match_model)$sum.matched["ATT"]
         } else {
              att <- NA  # Set ATT to NA if not found
         }
    
    # Extract balance information from the matching model summary
    smd_data <- summary(match_model)$covariate.balance
    #debugging print -- don't run with full sim
    print(smd_data) #missing (NULL)
...
simulation_results <- simulation_test$results
glimpse(simulation_results)
balance_plots <- simulation_test$balance_plots


# Plotting: ATT vs Proportion of Covariates Balanced -- sample
ggplot(simulation_results, aes(x = balanced_proportion, y = ATT)) +
  geom_point(alpha = 0.2) +  # Transparency to reduce overplotting
  labs(title = "ATT vs Proportion of Covariates Balanced", 
       x = "Proportion of Covariates Balanced (SMD <= 0.1)", 
       y = "Average Treatment Effect on the Treated (ATT)") +
  theme_minimal()

# Create random covariate balance plots using gridExtra (10 plots at a time)
grid.arrange(grobs = balance_plots, ncol = 5)

# Run the simulation with 10 iterations
simulation_output <- run_simulation(ypsps_clean, num_simulations = 1000)
glimpse(simulation_output)
```


```{r}
## running function on full data



#### full data
# Extract results -- full data
simulation_results <- simulation_output$results
balance_plots <- simulation_output$balance_plots

# Plotting: ATT vs Proportion of Covariates Balanced
ggplot(simulation_results, aes(x = balanced_proportion, y = ATT)) +
  geom_point(alpha = 0.2) +  # Transparency to reduce overplotting
  labs(title = "ATT vs Proportion of Covariates Balanced", 
       x = "Proportion of Covariates Balanced (SMD <= 0.1)", 
